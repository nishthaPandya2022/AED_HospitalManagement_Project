...
import model.Pathologist;
import model.PathologistDirectory;
import org.sqlite.SQLiteDataSource;
import static ui.admin.AllPathologistJPanel.sqliteConnection;

/**
 *
 * @author nishthapandya
 */
public class AllPathologistJPanel extends javax.swing.JPanel {
    
    private PathologistDirectory pathologistDirectory;    //doctorDirectory;
    private ArrayList<Pathologist> labAssistDirectory;     //docDirectory
    static SQLiteDataSource ds = null;
    static Connection sqliteConnection;

    String GET_ALL_LAB_ASSISTANT_FROM_HOSPITAL = "SELECT userID, name, DOB, age, organization, email, address, community, state, zipcode, role from hospital WHERE role=\"Pathologist\"";

    /**
     * Creates new form AllPathologistJPanel
     */
    public AllPathologistJPanel(SQLiteDataSource dataSource, Connection connection) {
        initComponents();
        
        this.ds = dataSource;
        this.sqliteConnection = connection;
        this.pathologistDirectory = new PathologistDirectory();
        labAssistDirectory = new ArrayList<Pathologist>();
        ...
        try {
            ...
            while (output.next()) {
                Pathologist doc = new Pathologist();
                doc.setPathologistID(Integer.parseInt(output.getString("userID")));
                doc.setPathologistName(output.getString("name"));
//                Date date = output.getDate("DOB");
                System.out.println("dob : " + output.getString("DOB"));
                doc.setPathologistDOB(java.sql.Date.valueOf(output.getString("DOB")));
                doc.setPathologistAge(output.getString("age"));
                doc.setPathologistEmail(output.getString("email"));
                doc.setPathologistAddress(output.getString("address"));
                doc.setPathologistCommunity(output.getString("community"));
                doc.setPathologistState(output.getString("state"));
                doc.setPathologistZipCode(output.getString("zipcode"));

                labAssistDirectory.add(doc);
            }

            this.pathologistDirectory.setPathologistDirectory(labAssistDirectory);

            populatePathologistTable();
    ...
    private void initComponents() {
        ...
        tableAllPathologist = new javax.swing.JTable();
        ...
        tableAllPathologist.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
        ...
        jScrollPane1.setViewportView(tableAllPathologist);
    ...
    private void bViewDoctorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bViewDoctorActionPerformed
        ...
        int selectedRow = tableAllPathologist.getSelectedRow();
        if (selectedRow < 0) {
            JOptionPane.showMessageDialog(this, "Please select a row to view !");
            return;
        }

        DefaultTableModel model = (DefaultTableModel) tableAllPathologist.getModel();
        Pathologist pathologist = (Pathologist) model.getValueAt(selectedRow, 0);

        etDoctorID.setText(String.valueOf(pathologist.getPathologistID()));
        etDoctorName.setText(pathologist.getPathologistName());
        etDoctorCity.setText(pathologist.getPathologistCity());
        dateChooserDocDOB.setDate(pathologist.getPathologistDOB());
        txtFieldDocAge.setText(pathologist.getPathologistAge());
        etDoctorCommunity.setText(pathologist.getPathologistCommunity());
        etDoctorZipCode.setText(pathologist.getPathologistZipCode());
        etDoctorState.setText(pathologist.getPathologistState());
        etDoctorAddress.setText(pathologist.getPathologistAddress());
        etDoctorEmail.setText(pathologist.getPathologistEmail());
    ...
    private void bDeleteDoctorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bDeleteDoctorActionPerformed
        ...
        pathologistDirectory.deletePathologist(pathologist);
        ...
        try {
            ...
            p2p.setInt(1, pathologist.getPathologistID());
            p2p.setString(2, "Pathologist");
    ...
    private void bUpdateDoctorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bUpdateDoctorActionPerformed
        ...
        if (validatedata()) {
            ...
            for (Pathologist pathologist : pathologistDirectory.getPathologistDirectory()) {

                int id = pathologist.getPathologistID();
                if (editDoctorID == id) {
                    pathologist.setPathologistID(editDoctorID);
                    pathologist.setPathologistDOB(editDoctorDOB);
                    pathologist.setPathologistAge(editDoctorAge);
                    pathologist.setPathologistName(editDoctorName);
                    pathologist.setPathologistAddress(editDoctorAddress);
                    pathologist.setPathologistCommunity(editDoctorCommunity);
                    pathologist.setPathologistCity(editDoctorCity);
                    pathologist.setPathologistState(editDoctorState);
                    pathologist.setPathologistZipCode(editDoctorZipCode);
                    pathologist.setPathologistEmail(editDoctorEmail);
    ...
    private javax.swing.JTable tableAllPathologist;
    private javax.swing.JTextField txtFieldDocAge;
    // End of variables declaration//GEN-END:variables

    public void populatePathologistTable() {

        DefaultTableModel model = (DefaultTableModel) tableAllPathologist.getModel();
        model.setRowCount(0);

        for (Pathologist doc : pathologistDirectory.getPathologistDirectory()) {
            Object[] row = new Object[4];

            row[0] = doc;
            row[1] = doc.getPathologistName();
            row[2] = doc.getPathologistEmail();
            row[3] = doc.getPathologistAge();
